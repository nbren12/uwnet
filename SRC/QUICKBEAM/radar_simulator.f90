...
! ----- OPTIONAL -----
!bloss(Jul 2015)  real*8, optional, dimension(nprof,ngate) :: &
! Make these arguments mandatory to appease intel compiler's complaints
!   about needing an explicit interface.
...
! ----- INTERNAL -----
  ...
  g_to_vol_in_present  = .true. !bloss present(g_to_vol_in)
  g_to_vol_out_present = .true. !present(g_to_vol_out)
