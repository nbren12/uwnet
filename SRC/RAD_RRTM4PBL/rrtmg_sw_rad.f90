...
! Control
      ...
      integer(kind=im), intent(in) :: iaer    ! aerosol option flag  -- bloss(RRTM4PBL): aerosol flag now an input
...
!bloss(RRTM4PBL): aerosol flag iaer is now an input
...
!  from GCM fields should be reversed here if necessary.
      ...
      pz(0) = plev(iplon,1)
      tz(0) = tlev(iplon,1)
!bloss(071609): changing limit of loop to nlayers-1 as suggested below.
      do l = 1, nlayers-1
...
!bloss(071609): adding layer to input soundings as suggested above.
      pavel(nlayers) = 0.5_rb * pz(nlayers-1)
      tavel(nlayers) = tavel(nlayers-1)
      pz(nlayers) = 1.e-4_rb
!bloss: extrapolated in rad_full
!bloss      tz(nlayers-1) = 0.5_rb * (tavel(nlayers)+tavel(nlayers-1))
...
!bloss(071609): Using this option, so uncommented below.
