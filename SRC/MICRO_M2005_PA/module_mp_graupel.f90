...
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
SUBROUTINE M2005MICRO_GRAUPEL(QC3DTEN,QI3DTEN,QNI3DTEN,QR3DTEN,QAD3DTEN,QAW3DTEN,QAR3DTEN, &
       NC3DTEN,NI3DTEN,NS3DTEN,NR3DTEN,NAD3DTEN,QC3D,QI3D,QNI3D,QR3D,QAD3D,QAW3D,QAR3D, &
       NC3D,NI3D,NS3D,NR3D,NAD3D,NUC3D,NUR3D,T3DTEN,QV3DTEN,T3D,QV3D,PRES,RHO,DZQ,W3D,WVAR,PRECRT,SNOWRT, &
...
!brnr: make tendencies for aerosol mass available
      REAL, DIMENSION(KMS:KME) ::  QAD3DTEN           ! DRY AEROSOL MASS MIXING RATIO TENDENCY (KG/KG/S)
...
!brnr: make tendencies for aerosol number available
      REAL, DIMENSION(KMS:KME) ::  NAD3DTEN           ! DRY AEROSOL NUMBER CONCENTRATION TENDENCY (1/KG/S)
...
!brnr: add mass for wet (rain and cloud) and dry aerosol     
      REAL, DIMENSION(KMS:KME) ::  QAD3D              ! DRY AEROSOL MASS MIXING RATIO (KG/KG)
...
!brnr: add number for wet and dry aerosol
      REAL, DIMENSION(KMS:KME) ::  NAD3D              ! DRY AEROSOL NUMBER CONCENTRATION (1/KG)
...
! BRNR ADDED 2/15/12 EXTRA DIAGNOSTIC AEROSOL TENDENCIES 
     REAL, DIMENSION(KMS:KME) ::  NACT      ! CHANGE N DUE TO ACTIVATION
     REAL, DIMENSION(KMS:KME) ::  QACT      ! CHANGE Q DUE TO ACTIVATION
...
! INIT LIMITER PROFILES TO ZERO
            ...
            NACT(KTS:KTE) = 0. !brnr track activated aerosol number tendency
            QACT(KTS:KTE) = 0. !brnr track activated aerosol mass tendency
            QAPRA(KTS:KTE) = 0.
            QAPRC(KTS:KTE) = 0.
            QAPRE(KTS:KTE) = 0.
            QASUBC(KTS:KTE) = 0.
...
! ZERO OUT PROCESS RATES
               ...
               NACT(K) = 0. !brnr track activated aerosol number tendency
               QACT(K) = 0. !brnr track activated aerosol mass tendency
               QAPRA(K) = 0.
               QAPRC(K) = 0.
               QAPRE(K) = 0.
               QASUBC(K) = 0.
...
!               DBGNAD(1) = DBGNAD(1) + NAD3D(K)
               ...
               IF (IPRGAER.EQ.1) THEN 
                  NANEW1 = (NAD3D(K) + NC3D(K))*RHO(K)
                  NANEW2 = 0. !Placeholder for further dry aerosol modes !brnr
                  MAER1 = (QAD3D(K) + QAW3D(K))*RHO(K)
                  MAER2 = 0. !Placeholder for further dry aerosol modes !brnr
                  IF (NANEW1.GT.0.) THEN
                     RM1 = (((6.*MAER1/(PI*RHOA*NANEW1))*EXP(-1.*9./2*LOG(SIG1)**2)))**(1/3.) ! calculate geometric mean radius (mode 1)
                  ELSE
                     RM1 = 0.
                     MAER1 = 0.
                  END IF
                  IF (NANEW2.GT.0.) THEN
                     RM2 = (((6.*MAER2/(PI*RHOA*NANEW2))*EXP(-1.*9./2*LOG(SIG2)**2)))**(1/3.) ! calculate geometric mean radius (mode 2)
                  ELSE
                     RM2 = 0.
                     MAER2 = 0.
                  END IF
...
! MAKE SURE THIS VALUE ISN'T GREATER THAN TOTAL NUMBER OF AEROSOL
                                 ...
                                 DUM2 = (DUM2-NC3D(K))/DT
                                 DUM2 = MAX(0.,DUM2)
                                 NACT(K) = DUM2 !brnr track aerosol activation tendency
                                 NC3DTEN(K) = NC3DTEN(K)+DUM2

                                 IF (IPRGAER.EQ.1) THEN
                                    NAD3DTEN(K) = NAD3DTEN(K)-DUM2
                                    DUM1 = MAER1/2.*(1.-DERF1(UU1-3.*SQRT(2.)/2.*LOG(SIG1)))
                                    DUM2 = MAER2/2.*(1.-DERF1(UU2-3.*SQRT(2.)/2.*LOG(SIG2)))
                                    DUM2 = (DUM1 + DUM2)/RHO(K)
                                    DUM2 = MIN((MAER1+MAER2)/RHO(K),DUM2) ! Make sure doesn't exceed total aerosol mass
                                    DUM2 = (DUM2-QAW3D(K))/DT
                                    DUM2 = MAX(0.,DUM2) ! Make sure activation doesn't remove mass
                                    QACT(K) = DUM2 !brnr track aerosol mass activation tendency
                                    QAW3DTEN(K) = QAW3DTEN(K)+DUM2
                                    QAD3DTEN(K) = QAD3DTEN(K)-DUM2
                                 END IF  ! IPRGAER
...
! MAKE SURE VALUE DOESN'T EXCEED THAT FOR NON-EQUILIBRIUM SS
                                 ...
                                 DUM2 = (DUM2-NC3D(K))/DT
                                 DUM2 = MAX(0.,DUM2)
                                 NACT(K) = DUM2 !brnr track activation number tendency
                                 NC3DTEN(K) = NC3DTEN(K)+DUM2
...
! AEROSOL MASS FOR EQUILIBRIUM SUPER SATURATION USING PROGNOSTIC AEROSOL
                                    ...
                                    IF (IPRGAER.EQ.1) THEN
                                       ...
                                       DUM2 = MAER2/2.*(1.-DERF1(UU2-3.*SQRT(2.)/2.*LOG(SIG2)))
                                       DUM2 = (DUM1 + DUM2)/RHO(K)
                                       DUMACTM = MIN((MAER1+MAER2)/RHO(K),DUM2) ! Make sure doesn't exceed total aerosol mass
                                    END IF  ! IPRGAER
...
! MAKE SURE ISN'T GREATER THAN NON-EQUIL. SS
                                    ...
                                    DUM2 = (DUM2-NC3D(K))/DT
                                    DUM2 = MAX(0.,DUM2)
                                    NACT(K) = DUM2 !brnr track aerosol number activation tendency
                                    NC3DTEN(K) = NC3DTEN(K)+DUM2

! AEROSOL MASS CALC FOR NON-EQUIL. SS, BOUND CHECK, AND TENDENCY
                                    IF (IPRGAER.EQ.1) THEN
                                       NAD3DTEN(K) = NAD3DTEN(K)-DUM2
                                       DUM1 = MAER1/2.*(1.-DERF1(UU1-3.*SQRT(2.)/2.*LOG(SIG1)))
                                       DUM2 = MAER2/2.*(1.-DERF1(UU2-3.*SQRT(2.)/2.*LOG(SIG2)))
...
! BASED ON TWOMEY 1959
                           ...
                           DUM2 = (DUM2-NC3D(K))/DT
                           DUM2 = MAX(0.,DUM2)
                           NACT(K) = DUM2 !brnr track activated number tendency
                           NC3DTEN(K) = NC3DTEN(K)+DUM2
                        ...
                        ELSE IF (IACT.EQ.2) THEN
                           ...
                           IF ((NANEW1.GT.0.).OR.(NANEW2.GT.0.)) THEN
                              ...
                              IF (IPRGAER.EQ.1) THEN
                                 ...
                                 DUM2 = (DUM2-QAW3D(K))/DT
                                 DUM2 = MAX(0.,DUM2) ! Make sure activation doesn't remove mass
                                 QACT(K) = DUM2 !brnr track activated mass tendency
                                 QAW3DTEN(K) = QAW3DTEN(K)+DUM2
                                 QAD3DTEN(K) = QAD3DTEN(K)-DUM2
                              END IF  ! IPRGAER
                           END IF ! (NANEW1.GT.0).OR.(NANEW2.GT.0)
