...
CONTAINS
   ...
   ! ENDDO
    ...
    IF (luse_standard_t_p) THEN
       DO n=1,npr
          ...
          CALL scavenging_coefficient(na,nc,1,1,1,xda,xdc,e_c, &
  ...
  SUBROUTINE m2011_scavenging(qna,qma,cmd,sigma, &
       & t,p,qv,rhw,rhi,                         &
       & qc,qr,qi,qs,qg,                         &
       & qnc,qnr,qni,qns,qng,                    &
    ...
    REAL(KIND=dp), DIMENSION(nmod), INTENT(IN) :: &
         sigma               ! Standard deviation of lognormal aerosol modes
    ...
    CALL scavenging_num(na,xda,ngh,xgh,wgh, &
         ie,je,ke,nmod,qna,cmd,sigma,g3d,dndt,dmdt)
  ...
  SUBROUTINE scavenging_num(na,da,ngh,xgh,wgh,ni,nj,nk,nmod,n,d,sigma,gamma,dndt,dmdt)
...
!                   gamma_int=coe*gamma(i,j,k,idx)+omcoe*gamma(i,j,k,idx+1)
  ...
  SUBROUTINE gaulag(x,w,n,alf)
    ...
    DO i = 1, n
       ...
       ELSEIF (i.EQ.2) THEN
          z = z+(15.+dp+6.25_dp*alf)/(1._dp+.9_dp*alf+2.5_dp*n)
       ELSE
          ai=i-2
