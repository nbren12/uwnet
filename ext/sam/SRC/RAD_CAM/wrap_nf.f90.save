...
! Author: Jim Rosinski
!
! $Id: wrap_nf.f90,v 1.1 2004/09/08 19:46:06 samcvs Exp $
!
!-------------------------------------------------------------------------------
...
!bloss#else
!bloss   ret = nf_put_att_double (nfid, varid, attname, xtype, len, attval)
!bloss#endif
   if (ret/=NF_NOERR) call handle_error (ret)
...
!bloss#else
!bloss   ret = nf_get_var_double (nfid, varid, arr)
!bloss#endif
   if (ret/=NF_NOERR) then
...
!bloss#else
!bloss   ret = nf_get_vara_double (nfid, varid, start, count, arr)
!bloss#endif
   if (ret/=NF_NOERR) then
...
!bloss#else
!bloss   ret = nf_put_var1_double (nfid, varid, index, val)
!bloss#endif
   if (ret/=NF_NOERR) call handle_error (ret)
...
!bloss#else
!bloss   ret = nf_put_vara_double (nfid, varid, start, count, arr)
!bloss#endif
   if (ret/=NF_NOERR) call handle_error (ret)
...
!bloss#else
!bloss   ret = nf_put_var_double (nfid, varid, arr)
!bloss#endif
   if (ret/=NF_NOERR) call handle_error (ret)
